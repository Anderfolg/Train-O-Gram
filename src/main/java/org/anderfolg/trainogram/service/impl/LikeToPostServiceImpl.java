...
public class LikeToPostServiceImpl implements LikeToPostService {
    ...
    @Override
    public List<Like> findAllLikesByPost( Long postID ) throws Status436PostDoesntExistException {
    ...
    @Override
    public void addLikeToPost( JwtUser jwtUser, Long postID ) throws Status440LikeAlreadyExistsException, Status436PostDoesntExistException, Status419UserException {
        User user = userService.findUserById(jwtUser.id());
        Post post = postService.findPostById(postID);
    ...
    @Override
    @Transactional
    public void deleteLikeFromPost( JwtUser jwtUser, Long postID ) throws Status438LikeDoesntExistException, Status436PostDoesntExistException, Status419UserException {
        User user = userService.findUserById(jwtUser.id());
        Post post = postService.findPostById(postID);
        if ( likeV2repository.existsByUserAndContentId(user, postID) ){
